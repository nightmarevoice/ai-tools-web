...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Google 注册失败,请重试')
      ...
      <Card className="w-full max-w-md shadow-lg">
        ...
        <CardContent className="space-y-4">
          ...
          <Button
            ...
            variant="outline"
            className="w-full h-11 flex items-center justify-center gap-3 hover:bg-slate-50"
          >
            {googleLoading ? (
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white px-2 text-slate-500">或使用邮箱注册</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={loading || googleLoading}
              className="w-full h-11 bg-[#0057FF] hover:bg-[#0057FF]/90"
            >
              {loading ? (
                <>
        ...
        <CardFooter className="flex flex-col space-y-2">
          <div className="text-sm text-center text-slate-600">
            已有账号?{' '}
            <Link href="/login" className="text-[#0057FF] hover:underline font-medium">
              立即登录
            </Link>
          </div>
        </CardFooter>
