...
export default function TestScreenshotPage() {
  ...
  // 检查图片大小（通过获取 blob）
  const checkImageSize = async (url: string): Promise<number> => {
    try {
      // 先尝试 HEAD 请求（更快，不下载完整图片）
      try {
      ...
      // 如果 HEAD 失败或没有 Content-Length，获取 blob
  ...
  const checkScreenshots = async () => {
    ...
    try {
      ...
      for (let i = 0; i < allApps.length; i++) {
        ...
        // 检查图片大小
        try {
          const size = await checkImageSize(app.screenshot_url)
    ...
    <div className="container mx-auto py-8 px-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            筛选 screenshot_url 为空或图片大小小于 10KB 的应用
          </CardDescription>
        </CardHeader>
